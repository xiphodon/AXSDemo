package login.qiyun.com.mydemo.view;
 ...
 * 自定义控件PagerTab
 * （导入PagerTab并解决相关报错）
 * 1. 创建BaseActivity继承AppCompatActivity（ActionbarActivity）(导入v7包)
 * 2. 创建状态选择器bg_tab_text和tab_text_color
 * 3. 自动导包
 * 4. 给UIUtils增加方法:getColorStateList,getDrawable
 */
public class PagerTab extends ViewGroup {
	...
	private void init() {
		//把一个值从dip转换成px
		mIndicatorHeight = Utils.dip2px(mIndicatorHeight);
		mDividerPadding = Utils.dip2px(mDividerPadding);
		mTabPadding = Utils.dip2px(mTabPadding);
		mDividerWidth = Utils.dip2px(mDividerWidth);
		mTabTextSize = Utils.dip2px(mTabTextSize);
		//创建一个scroller
	...
	private void addTextTab(final int position, String title) {
		...
		tab.setTextColor(Utils.getColorStateList(mTabTextColorResId));
		tab.setBackgroundDrawable(Utils.getDrawable(mTabBackgroundResId));
